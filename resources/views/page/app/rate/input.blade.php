<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bolder fs-3 mb-1">Master PHLN</span>
            <span class="text-muted mt-1 fw-bold fs-7">Rate</span>
        </h3>
    ...
    <div class="card-body py-3">
        <form id="form_input">
            ...
            <div class="row mt-5">
                <div class="col-lg-4">
                    <label class="required fs-6 fw-bold mb-2">Berlaku</label>
                    <input type="text" class="form-control" id="berlaku" name="berlaku" placeholder="Masukkan berlaku..." value="{{$data->awal}}">
                </div>
                <div class="col-lg-4">
                    <label class="required fs-6 fw-bold mb-2">S/D</label>
                    <input type="text" class="form-control" id="sd" name="sd" placeholder="Masukkan s/d..." value="{{$data->akhir}}">
                </div>
                <div class="col-lg-4">
                    <label class="required fs-6 fw-bold mb-2">Rate</label>
                    <input type="text" class="form-control" id="rate" name="rate" placeholder="Masukkan rate..." value="{{number_format($data->rate,2,",",".")}}">
                </div>
                <div class="min-w-150px mt-10 text-end">
                    @if ($data->id)
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('phln.rate.update',$data->id)}}','PATCH');" class="btn btn-sm btn-primary">Simpan</button>
                    @else
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('phln.rate.store')}}','POST');" class="btn btn-sm btn-primary">Simpan</button>
                    @endif
                </div>
            </div>
        </form>
