<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-5">
        <h3 class="card-title align-items-start flex-column">
            ...
            <span class="text-muted mt-1 fw-bold fs-7">Sektor</span>
        </h3>
            ...
            <div class="row">
                ...
                <div class="col-lg-6">
                    ...
                    <select class="form-select" id="tipe" name="tipe">
                        <option SELECTED DISABLED>Pilih Tipe Sektor</option>
                        <option value="Hibah" {{$data->tipe == 'Hibah' ? 'selected' : ''}}>Hibah</option>
                        <option value="Pinjaman" {{$data->tipe == 'Pinjaman' ? 'selected' : ''}}>Pinjaman</option>
                    </select>
                </div>
                <div class="min-w-150px mt-10 text-end">
                    @if ($data->id)
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('phln.sektor.update',$data->id)}}','PATCH');" class="btn btn-sm btn-primary">Simpan</button>
                    @else
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('phln.sektor.store')}}','POST');" class="btn btn-sm btn-primary">Simpan</button>
                    @endif
                </div>
            </div>
        </form>
