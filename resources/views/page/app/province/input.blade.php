<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-5">
        <h3 class="card-title align-items-start flex-column">
            ...
            <span class="text-muted mt-1 fw-bold fs-7">Provinsi</span>
        </h3>
    ...
    <div class="card-body py-3">
        <form id="form_input">
            <div class="row">
                <div class="col-lg-6">
                    <label class="required fs-6 fw-bold mb-2">No Provinsi</label>
                    <input type="text" class="form-control" name="no_prov" placeholder="Masukkan no prov..." value="{{$data->id_prov}}">
                </div>
                <div class="col-lg-6">
                    <label class="required fs-6 fw-bold mb-2">Nama Provinsi</label>
                    <input type="text" class="form-control" name="nm_prov" placeholder="Masukkan nama..." value="{{$data->nm_prov}}">
                </div>
                <div class="min-w-150px mt-10 text-end">
                    @if ($data->id)
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('phln.province.update',$data->id)}}','PATCH');" class="btn btn-sm btn-primary">Simpan</button>
                    @else
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('phln.province.store')}}','POST');" class="btn btn-sm btn-primary">Simpan</button>
                    @endif
                </div>
            </div>
        </form>
