<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-5">
        <h3 class="card-title align-items-start flex-column">
            ...
            <span class="text-muted mt-1 fw-bold fs-7">Donor</span>
        </h3>
        ...
        <form id="form_input">
            ...
            <div class="row mt-5">
                <div class="col-lg-6">
                    ...
                    <input type="text" class="form-control" name="singkatan" placeholder="Masukkan singkatan..." value="{{$data->singkatan}}">
                </div>
                <div class="col-lg-6">
                    <label class="required fs-6 fw-bold mb-2">No Register</label>
                    <input type="text" class="form-control" name="no_register" placeholder="Masukkan no register..." value="{{$data->no_register}}">
                </div>
                <div class="min-w-150px mt-10 text-end">
                    @if ($data->id)
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('phln.donor.update',$data->id)}}','PATCH');" class="btn btn-sm btn-primary">Simpan</button>
                    @else
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('phln.donor.store')}}','POST');" class="btn btn-sm btn-primary">Simpan</button>
                    @endif
                </div>
            </div>
        </form>
