        ...
        <form id="form_input">
            <div class="row">
                <div class="col-lg-6">
                    ...
                    <input type="hidden" class="form-control" name="department_id" placeholder="Masukkan id..." value="{{$department->id}}">
                    <input type="text" class="form-control" placeholder="Masukkan kode..." value="{{$department->nama}}" readonly>
                </div>
                <div class="col-lg-2">
                    <label class="required fs-6 fw-bold mb-2">Unor</label>
                    <input type="text" class="form-control" name="id_unor" placeholder="Masukkan kode..." value="{{$data->id_unor}}">
                </div>
                <div class="col-lg-4">
                ...
                <div class="min-w-150px mt-10 text-end">
                    @if ($data->id)
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('phln.unor.update',$data->id)}}','PATCH');" class="btn btn-sm btn-primary">Simpan</button>
                    @else
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('phln.unor.store')}}','POST');" class="btn btn-sm btn-primary">Simpan</button>
                    @endif
                </div>
            </div>
        </form>
