<div class="card mb-5 mb-xl-10">
    <div class="card-header border-0 pt-5">
        <h3 class="card-title align-items-start flex-column">
            ...
            <span class="text-muted mt-1 fw-bold fs-7">Kategori Kinerja</span>
        </h3>
        ...
        <form id="form_input">
            <div class="row">
                <div class="col-lg-4">
                    ...
                    <input type="text" class="form-control" name="kategori" placeholder="Masukkan kategori..." value="{{$data->kategori}}">
                </div>
                <div class="col-lg-4">
                    <label class="required fs-6 fw-bold mb-2">PV</label>
                    <input type="text" class="form-control" name="pv" placeholder="Masukkan pv..." value="{{$data->pv}}">
                </div>
                <div class="col-lg-4">
                    <label class="required fs-6 fw-bold mb-2">ETR</label>
                    <input type="text" class="form-control" name="etr" placeholder="Masukkan etr..." value="{{$data->etr}}">
                </div>
                <div class="min-w-150px mt-10 text-end">
                    @if ($data->id)
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('phln.kategori-kinerja.update',$data->id)}}','PATCH');" class="btn btn-sm btn-primary">Simpan</button>
                    @else
                    <button id="tombol_simpan" onclick="handle_save('#tombol_simpan','#form_input','{{route('phln.kategori-kinerja.store')}}','POST');" class="btn btn-sm btn-primary">Simpan</button>
                    @endif
                </div>
            </div>
        </form>
